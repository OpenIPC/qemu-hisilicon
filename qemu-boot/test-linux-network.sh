...
#   4. a DNS query from the guest's address -- guest-*initiated* traffic, out
#      through its default route
#
...
# 1 --- DHCP -----------------------------------------------------------------
GUEST_IP=""
for _ in $(seq 1 "$LOGIN_TIMEOUT"); do
    sudo chmod 0644 "$DNSMASQ_LOG" 2>/dev/null || true
...
if [ -n "$BANNER" ]; then
    ...
    say "3. host->guest TCP :22" "FAIL (no banner)"; FAIL=1
fi

# 4 --- guest-initiated outbound ----------------------------------------------
# busybox ntpd resolves its peers at S49, so a query reaching dnsmasq from the
# guest proves the default route and outbound UDP.  Any name will do -- which
# server the image is configured with is not this test's business.
QUERY=""
for _ in $(seq 1 "$CURL_TIMEOUT"); do
    sudo chmod 0644 "$DNSMASQ_LOG" 2>/dev/null || true
...
if [ -n "$QUERY" ]; then
    say "4. guest->out DNS query" "PASS -> ${QUERY#*: }"
else
    say "4. guest->out DNS query" "FAIL (no query from $GUEST_IP)"; FAIL=1
fi
...
echo "=== PASS: Linux network (DHCP + ICMP + TCP + outbound DNS) ==="
